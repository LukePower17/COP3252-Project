...
class LBoard
{
	...
	public boolean check(int player)
	{
		...
		if(player == 0)
		{
			//loop though the BlackAlive and get the tractory of each piece if corrdinates
			// are the same as the whit king's cors return true false;
	...
	public Vector<Vector<Integer>> getPawnTrajectory(String piece)
	{
		...
		if(player == 'b')
		{
				...
				// capture diagonally
				if(isValid(i - 1, j - 1) && (Board[i - 1][j - 1].charAt(0) - 'w') == 0)
				{
					Vector<Integer> c = new Vector<Integer>();
					c.add(i - 1);
					c.add(j - 1);
		...
		else
		{
			...
			// capture diagonally
			if(isValid(i + 1, j + 1) && (Board[i + 1][j + 1].charAt(0) - 'b') == 0)
			{
				Vector<Integer> c = new Vector<Integer>();
				c.add(i + 1);
				c.add(j + 1);
	...
	public Vector<Vector<Integer>> getKingTrajectory(String piece)
	{
		...
		Vector<Vector<Integer>> result = new Vector<Vector<Integer>>();
		if(x - 1 >= 0){
			if(Board[x - 1][y].equals("") == false && Board[x - 1][y].charAt(0) - player != 0)
			{
				Vector<Integer> cor = new Vector<Integer>();
				cor.add(x - 1);
				cor.add(y);
			...
			if(y - 1 >= 0){
				if(Board[x - 1][y - 1].equals("") == false  && Board[x - 1][y - 1].charAt(0) - player != 0)
				{
					Vector<Integer> cor = new Vector<Integer>();
					cor.add(x - 1);
					cor.add(y - 1);
		...
		if(y - 1 >= 0){
			if(Board[x][y - 1].equals("") == false  && Board[x][y - 1].charAt(0) - player != 0)
				{
					Vector<Integer> cor= new Vector<Integer>();
					cor.add(x);
					cor.add(y - 1);
